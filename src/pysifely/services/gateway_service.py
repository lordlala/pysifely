...
class GatewayService(BaseService):
    ...
    _subscribers: List[Tuple[Gateway, Callable[[Gateway], None]]] = []

    async def update(self, gateway: Gateway):
        # Get updated device_params
        async with BaseService._update_lock:
            gateway.device_params = await self.get_updated_params(gateway.mac)
        ...
        if (event := return_event_for_device(gateway, latest_events)) is not None:
            gateway.last_event = event
            gateway.last_event_ts = event.event_ts

        # Update gateway state
        state_response: List[Tuple[PropertyIDs, Any]] = await self._get_property_list(gateway)
        for property, value in state_response:
            if property is PropertyIDs.AVAILABLE:
                gateway.available = value == "1"
            if property is PropertyIDs.ON:
                gateway.on = value == "1"
            if property is PropertyIDs.CAMERA_SIREN:
                gateway.siren = value == "1"
            if property is PropertyIDs.FLOOD_LIGHT:
                gateway.floodlight = value == "1"
            if property is PropertyIDs.NOTIFICATION:
                gateway.notify = value == "1"
            if property is PropertyIDs.MOTION_DETECTION:
                gateway.motion = value == "1"

        return gateway

    async def register_for_updates(self, gateway: Gateway, callback: Callable[[Gateway], None]):
        loop = asyncio.get_event_loop()
        if not self._updater_thread:
            self._updater_thread = Thread(target=self.update_worker, args=[loop, ], daemon=True)
            self._updater_thread.start()

        self._subscribers.append((gateway, callback))

    async def deregister_for_updates(self, gateway: Gateway):
        self._subscribers = [(gate, callback) for gate, callback in self._subscribers if gate.mac != gateway.mac]
    ...
    def update_worker(self, loop):
        while True:
            if len(self._subscribers) < 1:
                ...
                for gateway, callback in self._subscribers:
                    try:
                        callback(asyncio.run_coroutine_threadsafe(self.update(gateway), loop).result())
    ...
    async def get_gateways(self) -> List[Gateway]:
        ...
        gateways = [device for device in self._devices if device.type is DeviceTypes.GATEWAY]

        return [Gateway(gateway.raw_dict) for gateway in gateways]

    #async def turn_on(self, camera: Gateway):
    #    await self._run_action(gateway, "power_on")
